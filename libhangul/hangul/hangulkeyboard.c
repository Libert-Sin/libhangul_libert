...
static const HangulKeyboard hangul_keyboard_3libert = {
    (char*)"3l",
    (char*)N_("Sebeolsik Libert"),
    { (ucschar*)hangul_keyboard_table_3libert, NULL, NULL, NULL },
    { (HangulCombination*)&hangul_combination_default, NULL, NULL, NULL },
    HANGUL_KEYBOARD_TYPE_JASO,
    true
};
...
static void hangul_keyboard_list_clear() {
    size_t i;
    for (i = 0; i < hangul_keyboards.n; ++i) {
        printf("Deleting keyboard: %s\n", hangul_keyboards.keyboards[i]->id); // 로그 추가
...
int hangul_keyboard_list_init() {
    ...
    while (dir != NULL && dir[0] != '\0') {
        ...
        printf("Loading keyboards from directory: %s\n", dir); // 로그 추가
